...
setup:
	...
	if [ ! -d lib/bootstrap ]; then curl -O -L https://github.com/twbs/bootstrap/releases/download/v3.3.6/bootstrap-$(bootstrap_version)-dist.zip && unzip bootstrap-$(bootstrap_version)-dist.zip && mv bootstrap-$(bootstrap_version)-dist lib/bootstrap && rm bootstrap-$(bootstrap_version)-dist.zip; fi;
